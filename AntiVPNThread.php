...
namespace cosmicpe\novpn\libs\cosmoverse\antivpn\thread;

use ClassLoader;
use Closure;
use cosmicpe\novpn\libs\cosmoverse\antivpn\api\AntiVPNRequest;
use cosmicpe\novpn\libs\cosmoverse\antivpn\thread\request\AntiVPNRequestCallback;
use cosmicpe\novpn\libs\cosmoverse\antivpn\thread\request\AntiVPNRequestHolder;
use cosmicpe\novpn\libs\cosmoverse\antivpn\thread\result\AntiVPNResultHolder;
use cosmicpe\novpn\libs\cosmoverse\antivpn\thread\result\AntiVPNResultTypeFailure;
use cosmicpe\novpn\libs\cosmoverse\antivpn\thread\result\AntiVPNResultTypeSuccess;
use Exception;
use Logger;
			...
			while(($incoming = $this->incoming->shift()) !== null){
				...
				if($result !== false){
					try{
						...
						$result = new AntiVPNResultTypeFailure(new AntiVPNException("Failed when decoding json: \"" . $result . "\": " . $e->getMessage()));
					}
				}else{
